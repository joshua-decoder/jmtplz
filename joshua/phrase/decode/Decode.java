...
public class Decode {
  ...
  public Decode(Context context, PhraseTable table, String in,
    ...
    Chart chart = new Chart(table, in, context.GetVocab(), context.GetScorer());
  ...
  public void Output(Hypothesis hypo, Vocabulary vocab, OutputStreamWriter to) throws IOException {
    ...
    for (int i = hypos.size() - 1; i >= 0; i--) { // todo: skip EOS?
      for (long id : hypos.get(i).Target().getWords()) {
        to.write(" " + vocab.String(id));
      }
    }
  }
  
  public void OutputVerbose(Hypothesis hypo, Vocabulary vocab, HashMap<String,ScoreHistory> map, OutputStreamWriter to) throws IOException {
    List<Hypothesis> hypos = new ArrayList<Hypothesis>();
    for (Hypothesis h = hypo; h != null; h = h.Previous()) {
      if (!h.Target().Valid()) continue;
      hypos.add(h);
