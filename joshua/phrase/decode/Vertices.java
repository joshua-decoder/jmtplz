...
public class Vertices {
  ...
  public void Add(Hypothesis hypothesis, int source_begin, int source_end, float score_delta) {
    ...
    StacksShared.AddHypothesisToVertex(hypothesis, score_delta, map.get(key));
  }
  ...
  public void Apply(Chart chart, EdgeGenerator out) {
    for (IntPair pair : map.keySet()) {
      Note note = new Note();
      note.ints = pair; // i
      StacksShared.AddEdge(map.get(pair), chart.Range(pair.first, pair.second).getVertex(), note, out);
    }
  }
